...
namespace PhysiK
{
    namespace
    {
        ...
        float MaxAbsDifferenceFromTranspose(const Mat3& reverse, const Mat3& forward)
        {
            float maxDifference = 0.0f;
            maxDifference = std::max(
                maxDifference,
                std::abs(reverse.columns[0].x - forward.columns[0].x));
            maxDifference = std::max(
                maxDifference,
                std::abs(reverse.columns[1].x - forward.columns[0].y));
            maxDifference = std::max(
                maxDifference,
                std::abs(reverse.columns[2].x - forward.columns[0].z));
            maxDifference = std::max(
                maxDifference,
                std::abs(reverse.columns[0].y - forward.columns[1].x));
            maxDifference = std::max(
                maxDifference,
                std::abs(reverse.columns[1].y - forward.columns[1].y));
            maxDifference = std::max(
                maxDifference,
                std::abs(reverse.columns[2].y - forward.columns[1].z));
            maxDifference = std::max(
                maxDifference,
                std::abs(reverse.columns[0].z - forward.columns[2].x));
            maxDifference = std::max(
                maxDifference,
                std::abs(reverse.columns[1].z - forward.columns[2].y));
            maxDifference = std::max(
                maxDifference,
                std::abs(reverse.columns[2].z - forward.columns[2].z));
            return maxDifference;
        }

        void LogSymmetryDiagnostic(const SparseBlockMatrix& matrix)
        {
            int checkedPairs = 0;
            int missingPairs = 0;
            int badPairs1eMinus5 = 0;
            int badPairs1eMinus4 = 0;
            int badPairs1eMinus3 = 0;
            double totalAbsDifference = 0.0;
            ...
            for (int rowBlock = 0; rowBlock < matrix.blockCount; ++rowBlock)
            {
                ...
                for (int blockIndex = rowBegin; blockIndex < rowEnd; ++blockIndex)
                {
                    ...
                    const int reverseIndex = matrix.FindBlockIndex(columnBlock, rowBlock);
                    if (reverseIndex < 0)
                    {
                        ++missingPairs;
                        continue;
                    ...
                    const Mat3& reverse =
                        matrix.values[static_cast<std::size_t>(reverseIndex)];
                    const float difference =
                        MaxAbsDifferenceFromTranspose(reverse, forward);

                    ++checkedPairs;
                    totalAbsDifference += static_cast<double>(difference);
                    maxAbsDifference = std::max(
                        maxAbsDifference,
            ...
            constexpr const char* DiagnosticPath =
                "PhysiK_SparseBlockMatrix_Symmetry.csv";
            ...
            if (!file.is_open())
            {
                ...
                {
                    file
                        << "pattern,blocks,nonZeroBlocks,checkedPairs,"
                        << "missingPairs,maxAbsDifference,averageAbsDifference,"
                        << "badPairs1eMinus5,badPairs1eMinus4,badPairs1eMinus3\n";
                }
            }

            const double averageAbsDifference =
                checkedPairs > 0
                    ? totalAbsDifference / static_cast<double>(checkedPairs)
                    : 0.0;
